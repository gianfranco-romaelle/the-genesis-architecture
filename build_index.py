...
# Each child chunk gets a situating description prepended before embedding so
# the vector captures document identity and topic, not just surface text.
#
# CONTEXT_PROVIDER options:
#   "metadata"   — free, always available; constructs context from Zotero/filename
#                  metadata (title, author, year, page). No API calls. DEFAULT.
#   "claude"     — paid; Claude Haiku generates a 1-2 sentence semantic description
#                  with prompt caching (~$0.0003-0.0007/chunk).
#                  [Recommended by Claude Sonnet 4.6 as best quality/cost option]
#   "openrouter" — free tier but rate-limited (50 req/day); uses OPENROUTER_API_KEY.
#                  [Claude Sonnet 4.6: viable for small test batches only]
#   "none"       — disable context entirely (same as --no-context flag)
#
# Paid model config (Claude Sonnet 4.6 recommendations):
#   Best quality/cost: "claude-haiku-4-5-20251001"  (~$0.0003-0.0007/chunk w/ caching)
#   Best free local:   run Ollama + "qwen2.5:3b"    (no API cost, needs ~3 GB RAM)
CURRENT_CONTEXT_VERSION = 1       # increment to force re-contextualization of all files
CONTEXT_PROVIDER = "metadata"     # see options above; change to "claude" to enable LLM context
CONTEXT_CLAUDE_MODEL = "claude-haiku-4-5-20251001"   # [Claude Sonnet 4.6 suggestion]
CONTEXT_OPENROUTER_MODEL = "meta-llama/llama-3.3-70b-instruct:free"  # [Claude Sonnet 4.6 suggestion]
CONTEXT_DOC_CHARS = 4000          # document beginning used as context for LLM providers
CONTEXT_MAX_TOKENS = 120          # max output tokens per context string (LLM providers)
_CONTEXT_ENABLED = True           # set to False via --no-context
...
_anthropic_context_client: Optional[Any] = None
_anthropic_context_init_tried: bool = False
_openrouter_context_client: Optional[Any] = None
_openrouter_context_init_tried: bool = False

def _get_anthropic_for_context() -> Optional[Any]:
    global _anthropic_context_client, _anthropic_context_init_tried
    if not ANTHROPIC_OK or _anthropic_context_init_tried:
        return _anthropic_context_client
    _anthropic_context_init_tried = True
    key = _get_env_key("ANTHROPIC_API_KEY")
    if key:
        _anthropic_context_client = _anthropic_lib.Anthropic(api_key=key)
    else:
        console.print("[yellow]ANTHROPIC_API_KEY not set — 'claude' context provider unavailable[/yellow]")
    return _anthropic_context_client


def _get_openrouter_for_context() -> Optional[Any]:
    global _openrouter_context_client, _openrouter_context_init_tried
    if _openrouter_context_init_tried:
        return _openrouter_context_client
    _openrouter_context_init_tried = True
    try:
        import openai as _openai_lib
        key = _get_env_key("OPENROUTER_API_KEY")
        if key:
            _openrouter_context_client = _openai_lib.OpenAI(
                api_key=key,
                base_url="https://openrouter.ai/api/v1",
                max_retries=0,
            )
        else:
            console.print("[yellow]OPENROUTER_API_KEY not set — 'openrouter' context provider unavailable[/yellow]")
    except ImportError:
        console.print("[yellow]openai package not installed — 'openrouter' context provider unavailable[/yellow]")
    return _openrouter_context_client


_CONTEXT_SYSTEM = (
    "You are a research assistant helping index a historical scientific library. "
    "Given a document excerpt and a passage from that document, write 1–2 sentences "
    "situating the passage within the broader work. Include the document's subject, "
    "the section or argument this passage belongs to, and any key entities (people, "
    "concepts, experiments) visible in the surrounding context. "
    "Be specific and concise. Output ONLY the context description."
)


def _doc_identity(source_meta: dict) -> str:
    file_name = source_meta.get("file_name", "unknown")
    title = source_meta.get("title", "")
    author = source_meta.get("author", "")
    year = source_meta.get("year", "")
    id_parts = [file_name]
    if title and title.lower() != Path(file_name).stem.lower():
        id_parts.append(title)
    if author:
        id_parts.append(f"by {author}")
    if year:
        id_parts.append(str(year))
    return " — ".join(id_parts)


def _ctx_metadata(chunks: list[dict], source_meta: dict) -> None:
    """Free, always-available provider: constructs context from bibliographic metadata."""
    title = source_meta.get("title") or Path(source_meta.get("file_name", "")).stem
    author = source_meta.get("author", "")
    year = source_meta.get("year", "")
    ...
    doc_parts: list[str] = []
    if author:
        doc_parts.append(str(author))
    if title:
        doc_parts.append(str(title))
    if year:
        doc_parts.append(f"({year})")
    doc_str = ", ".join(doc_parts) if doc_parts else source_meta.get("file_name", "unknown")
    total = f" of {page_count}" if page_count else ""

    for chunk in chunks:
        page = chunk.get("page_number", "")
        page_str = f", p. {page}{total}" if page else ""
        chunk["context"] = f"From: {doc_str}{page_str}."


def _ctx_claude(chunks: list[dict], doc_header: str, source_meta: dict) -> None:
    """Paid provider: Claude Haiku with prompt caching (~$0.0003-0.0007/chunk)."""
    client = _get_anthropic_for_context()
    if not client:
        _ctx_metadata(chunks, source_meta)  # graceful fallback
        return
    doc_id = _doc_identity(source_meta)
    cached_text = (
        f"<document_identity>{doc_id}</document_identity>\n\n"
        f"<document_beginning>\n{doc_header[:CONTEXT_DOC_CHARS]}\n</document_beginning>"
    )
    for chunk in chunks:
        snippet = chunk.get("text", "")[:600]
        try:
            resp = client.messages.create(
                model=CONTEXT_CLAUDE_MODEL,
                max_tokens=CONTEXT_MAX_TOKENS,
                system=[{"type": "text", "text": _CONTEXT_SYSTEM,
                         "cache_control": {"type": "ephemeral"}}],
                messages=[{"role": "user", "content": [
                    {"type": "text", "text": cached_text,
                     "cache_control": {"type": "ephemeral"}},
                    {"type": "text", "text": (
                        f"<passage_to_situate>\n{snippet}\n</passage_to_situate>"
                        "\n\nContext description:"
                    )},
                ]}],
            )
            chunk["context"] = resp.content[0].text.strip()
        except Exception as exc:
            console.print(f"[dim]Context skip (claude): {exc}[/dim]")
            chunk["context"] = ""


def _ctx_openrouter(chunks: list[dict], doc_header: str, source_meta: dict) -> None:
    """Free-tier provider: OpenRouter free models (rate-limited to 50 req/day)."""
    client = _get_openrouter_for_context()
    if not client:
        _ctx_metadata(chunks, source_meta)
        return
    doc_id = _doc_identity(source_meta)
    system_content = _CONTEXT_SYSTEM
    doc_content = (
        f"<document_identity>{doc_id}</document_identity>\n\n"
        f"<document_beginning>\n{doc_header[:CONTEXT_DOC_CHARS]}\n</document_beginning>"
    )
    for chunk in chunks:
        ...
        try:
            ...
            chunk["context"] = resp.choices[0].message.content.strip()
        except Exception as exc:
            console.print(f"[dim]Context skip (openrouter): {exc}[/dim]")
            chunk["context"] = ""
...
def contextualize_chunks(
    ...
    """Dispatch to the configured CONTEXT_PROVIDER. Falls back to metadata on any failure."""
    if not chunks:
        return
    if CONTEXT_PROVIDER == "claude":
        _ctx_claude(chunks, doc_header, source_meta)
    elif CONTEXT_PROVIDER == "openrouter":
        _ctx_openrouter(chunks, doc_header, source_meta)
    else:  # "metadata" or anything unrecognised
        _ctx_metadata(chunks, source_meta)
...
def process_file(
    ...
    # ── Contextual retrieval ────────────────────────────────────────────────────
    # Generate a 1-2 sentence situating description for each chunk via Claude Haiku
    # (prompt-cached per document). The context is prepended to the embedding text
    # so vector search captures document position and topic, not just surface words.
    # Original chunk text is stored in the payload unchanged for citation/display.
    if _CONTEXT_ENABLED:
        contextualize_chunks(index_chunks, full_text[:CONTEXT_DOC_CHARS], source_meta)

    if client is None:
        result["status"] = "embedded_no_qdrant"
        result["chunks_indexed"] = len(index_chunks)
        result["context_version"] = CURRENT_CONTEXT_VERSION if _CONTEXT_ENABLED else 0
        return result

    total_upserted = 0
    # Embed context + chunk text; payload["text"] keeps the original for display.
    embed_texts = [
...
def main() -> None:
    ...
    ap.add_argument("--no-context", action="store_true",
                    help="Disable contextual retrieval entirely; embed raw chunk text only")
    ap.add_argument("--context-provider",
                    choices=["metadata", "claude", "openrouter", "none"],
                    default=None,
                    help="Context generation provider (default: metadata). "
                         "'metadata' is free; 'claude'/'openrouter' require API keys. "
                         "[Model suggestions by Claude Sonnet 4.6]")
    ...
    args = ap.parse_args()
    global _DOCLING_ENABLED, _CONTEXT_ENABLED, CONTEXT_PROVIDER
    if args.no_docling:
        _DOCLING_ENABLED = False
        console.print("[yellow]Docling OCR disabled (--no-docling)[/yellow]")
    if args.no_context or (args.context_provider and args.context_provider == "none"):
        _CONTEXT_ENABLED = False
        console.print("[yellow]Contextual retrieval disabled[/yellow]")
    elif args.context_provider:
        CONTEXT_PROVIDER = args.context_provider
        console.print(f"[dim]Context provider: {CONTEXT_PROVIDER}[/dim]")
