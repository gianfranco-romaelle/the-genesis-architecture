...
# Each child chunk gets a 1-2 sentence situating description generated by Claude
# Haiku before embedding. The context is prepended to the chunk text so the
# embedding captures document position and topic, not just surface text.
# Cost: ~$0.0003–0.0007 per chunk with prompt caching (doc header is cached
# across all chunks in the same document).
CURRENT_CONTEXT_VERSION = 1       # increment to force re-contextualization of all files
CONTEXT_MODEL = "claude-haiku-4-5-20251001"
CONTEXT_DOC_CHARS = 4000          # document beginning cached as context for Haiku
CONTEXT_MAX_TOKENS = 120          # max output tokens per context string
...
def get_anthropic_for_context() -> Optional[Any]:
    ...
    if key:
        ...
        console.print("[yellow]ANTHROPIC_API_KEY not set — contextual retrieval disabled[/yellow]")
    return _anthropic_context_client
...
def contextualize_chunks(
    ...
    """
    Add a 'context' field to each chunk in-place (1-2 sentence situating description).
    Uses Anthropic prompt caching: the document header is cached across all chunks in
    the same document, so only the first call pays full input token cost.
    Chunks for which the API call fails get context="" and are still indexed normally.
    """
    client = get_anthropic_for_context()
    ...
    doc_identity = " — ".join(id_parts)

    # This block is marked cacheable; Haiku caches everything up to and including
    # the last cache_control block, so the doc header is reused across chunk calls.
    cached_text = (
        f"<document_identity>{doc_identity}</document_identity>\n\n"
    ...
    for chunk in chunks:
        ...
        try:
            ...
            console.print(f"[dim]Context skip: {exc}[/dim]")
            chunk["context"] = ""
...
def main() -> None:
    ...
    ap.add_argument("--no-context", action="store_true",
                    help="Disable Anthropic contextual retrieval; embed raw chunk text only")
    ...
    args = ap.parse_args()
    global _DOCLING_ENABLED, _CONTEXT_ENABLED
    if args.no_docling:
        _DOCLING_ENABLED = False
        console.print("[yellow]Docling OCR disabled (--no-docling)[/yellow]")
    if args.no_context:
        _CONTEXT_ENABLED = False
        console.print("[yellow]Contextual retrieval disabled (--no-context)[/yellow]")
